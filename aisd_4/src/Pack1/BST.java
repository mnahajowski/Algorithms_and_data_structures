...
public class BST {
	...
	public String search_value(Node current, int value) {
		if(current == null)
			return "Brak takiego elementu";
		if(current.getKey() == value)
			return current.getId();
		else if(value < current.getKey())
			return search_value(current.getLeft(), value);
		else
			return search_value(current.getRight(), value);
	}
	
	public Node search_value_2(Node current, int value) {
		if(current == null)
			return current;
		if(current.getKey() == value)
			return current;
		else if(value < current.getKey())
			return search_value_2(current.getLeft(), value);
		else
			return search_value_2(current.getRight(), value);
	}
	...
	public void subtree(int value) {
		show_inorder(search_value_2(root, value));
	}
